...
Pass-1 config freezes the existing QLoRA model/LoRA/optimizer settings from `configs/train-t4.yaml`, except: run `v3-pass1-t4`; data directory `data/processed/v3`; total cap 8,000; max length 768; max steps 500; the five probability buckets; hub pushes disabled. Rank stays 16, alpha 32, dropout 0.05, learning rate `1e-4`, cosine schedule, warmup 0.03, micro-batch 1, accumulation 16.
