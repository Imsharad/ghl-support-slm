...
The hiring brief is authoritative. The owner explicitly authorizes autonomous local implementation, design changes, data preparation, free-compute training, evaluation, local serving, README/demo preparation, and local commits. Spending is $0. Publication, pushes, merges to main, employer submission, and deletion of historical work remain excluded.
...
4. Train a candidate on a verified free Colab/Kaggle accelerator, saving adapters, full run inputs, curves, and recovery evidence. Select using development results; preserve unsuccessful experiments. No unconditional second pass or per-update 60-item remote-judge loop is required.
...
Access observation: `colab sessions` on this turn failed before listing sessions because Google OAuth reauthorization is required. No new runtime or generation was launched. Local work continues; an asynchronous request asks the owner to restore Colab access.
