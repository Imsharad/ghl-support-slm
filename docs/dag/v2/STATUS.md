...
| A1 | Opus | `tools/admissions.py`, `data/prepare.py` (`cap_train_rows`, assemble path), `tests/test_prepare.py`, `tests/test_admissions.py`, `data/v2/admissions.jsonl`, `data/v2/admission_review.jsonl`, `data/v2/leakage.json`, `data/v2/cap.json`, `docs/v2/DATA_V2.md` (append) | Gate1 seal | dry run 18:30, rows 19:45 | one-intent dry run, then `uv run python data/prepare.py --audit-only --strict` | first run (18:44, parallel 4) measured at 3.3 rows/min, finish ~21:00, restart ordered 18:59 IST: per-cell append, seeded interleave, parallel 12, 20:10 cutoff under decision 10; closer-concentration count pre-registered 18:52 IST, see note; generator + `tools/closer_audit.py` at 8cfd6f3, 67 tests | `8cfd6f3` (partial) |
| A2 | Grok | admission-row review (path per A1 brief) | A1 | ~19:45 | lint rules from `docs/v2/PLAN_DECISIONS.md` sec 4 | briefed 18:58 IST (owned: `data/v2/admission_review_a2.jsonl`, `docs/v2/ADMISSION_REVIEW_A2.md`, branch `grok/v2-a2`); starts on Opus's rows post, due 35 min after | — |
