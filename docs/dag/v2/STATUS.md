...
| Train launch | Fable (Mac mps run `v2-mps` from 02:10 IST as insurance; Colab T4 notebook run `v2-t4` restarted 02:13 IST after the first VM was reclaimed at 02:10 at step ~190) | `train/runs/v2-mps`, `train/runs/v2-t4` | Re-audit | Colab ~04:20, Mac ~05:00 Tue | `tools/check_run.py <run> --max-memory-gb 12` | both live; headline rule: Colab if clean and downloaded by 09:00, else the Mac run | — |
| Checkpoint/merge | Sol (dispatch when the zip lands), Fable fallback | checkpoint selection on dev (Grok by shell-out judges), merge, Q8 quant, Ollama tag `ghl-support-v2` | Train launch | ~04:00 Tue | manual smoke of served model; `tools/check_artifacts.py` | blocked on Train launch | — |
