...
| Train launch | Fable (Colab CLI, `--auth adc`, session `ghl-v2`) | `train/runs/v2-t4` on the VM, zip downloaded to the Mac | Re-audit | launch ~01:15 Tue | `tools/check_run.py train/runs/v2-t4 --max-memory-gb 12` on the VM | T4 session up since 23:29 IST; v2 prepare and CUDA smoke run proven on the VM (resume ok 0.00177); Mac insurance run not viable, swap 12 of 14 GB | — |
