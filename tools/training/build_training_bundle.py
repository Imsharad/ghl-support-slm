...
def bundle_sources(root: Path, config_path: Path) -> list[Path]:
    ...
    for split in ("train", "val"):
        ...
        paths.append(path)
    for name in ("pyproject.toml", "uv.lock", "README.md", "configs/models/versions.json", "configs/prompts/prompt.txt",
                 config["prompt_file"], "data/prepare.py", "tools/training/check_run.py", "tools/training/run_training_bundle.py",
