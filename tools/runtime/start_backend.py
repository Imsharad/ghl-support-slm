...
def install_ollama():
    config = json.loads((ROOT / 'configs/runtime/bootstrap.json').read_text())
...
def main():
    ...
    try:
        ...
        subprocess.run([sys.executable, 'tools/artifacts/fetch_artifacts.py', '--manifest', 'artifacts/v3/manifest.json', '--target', 'serve'], cwd=ROOT, check=True)
        env = os.environ.copy()
        ...
        manifests = json.loads((ROOT / 'configs/runtime/bootstrap.json').read_text())['model_manifests']
