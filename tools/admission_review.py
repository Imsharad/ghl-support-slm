...
and prompt hash land on every verdict line.
    ...
    uv run python tools/admission_review.py --input data/v2/admissions_raw.jsonl \
...
def write_report(path: Path, rows: list[dict], lines: list[dict], input_path: Path, input_sha: str) -> None:
    ...
    try:
        closer = subprocess.run(
            ["uv", "run", "python", "tools/closer_audit.py", str(input_path)],
    ...
    md = [
        ...
        "",
        f"Reviewer: `{REVIEW_MODEL}` via `grok -p`, restricted prompt (`tools/admission_review.py`), "
