...
from tools.training.build_training_bundle import bundle_sources, create_bundle, inside
from tools.training.run_training_bundle import verify
...
def source(tmp_path):
    root = tmp_path / "repo"
    root.mkdir()
    files = ["pyproject.toml", "uv.lock", "README.md", "configs/models/versions.json", "configs/prompts/prompt.txt",
             "configs/custom.txt", "data/prepare.py", "tools/training/check_run.py", "tools/training/run_training_bundle.py",
             "tools/training/check_cuda_host.py",
    ...
    config_path = root / "configs/training/train.yaml"
...
def test_bundle_is_deterministic_and_excludes_test_data_and_credentials(source, tmp_path):
    ...
    a = create_bundle(source, Path("configs/training/train.yaml"), first, "fixture-commit")
    b = create_bundle(source, Path("configs/training/train.yaml"), second, "fixture-commit")
...
def test_bundle_rejects_changed_data_publication_and_external_paths(source):
    ...
    with pytest.raises(ValueError, match="corpus manifest"):
        bundle_sources(source, Path("configs/training/train.yaml"))
    with pytest.raises(ValueError, match="escapes"):
        inside(source, source.parent / "outside.txt")
    config_path = source / "configs/training/train.yaml"
